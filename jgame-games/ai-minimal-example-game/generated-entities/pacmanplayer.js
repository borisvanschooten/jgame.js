/*@webcogs_build 0.6.0 openai-gpt-5.4 2026-05-11T07:58:25.655Z
...
@webcogs_user_prompt
Create a player that moves through a maze like Pac-man using the left stick. It moves through 'empty' tiles and pickups but stops at 'wall' tiles. Dies when it hits an enemy.
@webcogs_end_prompt_section*/
export class Player extends GameEntity {
    constructor(name, unique, tx, ty, mask, sprite, {moveDelay=6}={}) {
        super(name, unique, tx, ty, mask, sprite);
        this.moveDelay = moveDelay;
        this.pendingDx = 0;
        this.pendingDy = 0;
        this.wallMask = getTileMaskDef('wall');
        this.emptyMask = getTileMaskDef('empty');
    }

    hit(entity) {
        if (!entity) return;
        if ((entity.getMask() & enemy_mask) !== 0) {
            this.remove();
            gameOver();
        }
    }
    ...
    moveTick() {
        const axes = io.getAxes(0);
        this.pendingDx = axes.x;
        this.pendingDy = axes.y;
    ...
    moveTile() {
        let dx = this.pendingDx;
        let dy = this.pendingDy;

        if (dx === 0 && dy === 0) {
            return;
        }

        const env = this.getEnv(dx, dy);
        if ((env & this.wallMask) !== 0) {
            return;
        }

        this.goTo(this.getX() + dx, this.getY() + dy, this.moveDelay);
    }
}