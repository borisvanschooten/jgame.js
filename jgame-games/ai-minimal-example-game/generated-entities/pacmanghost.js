/*@webcogs_build 0.6.0 openai-gpt-5.4 2026-05-11T07:58:30.763Z
...
@webcogs_user_prompt
Create an enemy that moves randomly through a maze like a Pac-man ghost. It moves through 'empty' tiles but stops at 'wall' tiles.
@webcogs_end_prompt_section*/
export class MazeGhost extends GameEntity {
    constructor(name, unique, tx, ty, mask, sprite, {moveDelay=8, emptyMaskName='empty', wallMaskName='wall'}={}) {
        super(name, unique, tx, ty, mask, sprite);
        this.moveDelay = moveDelay;
        this.emptyMask = getTileMaskDef(emptyMaskName);
        this.wallMask = getTileMaskDef(wallMaskName);
        this.directions = [
            {x: 1, y: 0},
            {x: -1, y: 0},
            {x: 0, y: 1},
    ...
    moveTile() {
        ...
        for (let i = 0; i < this.directions.length; i++) {
            ...
            const env = this.getEnv(dir.x, dir.y);
            const blockedByWall = (env & this.wallMask) !== 0;
            const allowsEmpty = this.emptyMask === 0 || (env & this.emptyMask) !== 0;

            if (!blockedByWall && allowsEmpty) {
                options.push(dir);
            }
        }
        ...
        if (this.dir !== null) {
            ...
            for (let i = 0; i < options.length; i++) {
                if (options[i].x === this.dir.x && options[i].y === this.dir.y) {
                    forwardAvailable = true;
                    break;
                }
            }
            if (forwardAvailable && random2(0, 1) < 0.6) {
                chosen = this.dir;
            }
        }

        if (chosen === null) {
            const index = randomstep2(0, options.length - 1, 1);
            chosen = options[index];
        }

        this.dir = {x: chosen.x, y: chosen.y};
        this.goTo(this.getX() + chosen.x, this.getY() + chosen.y, this.moveDelay);
    }
}